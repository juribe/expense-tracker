...
result = process(Expenses::Input.from_params("audio", audio_data: AUDIO_DATA,
    ...
    test "the transcript and provider metadata are recorded in the stt step" do
      stub_transcription do
        result = process(Expenses::Input.from_params("audio", audio_data: AUDIO_DATA, metadata: { filename: "note.ogg" }))
    ...
    test "unsupported audio input is rejected before speech-to-text runs" do
      result = process(Expenses::Input.from_params("audio", audio_data: "data:video/mp4;base64,AAAA", metadata: { filename: "clip.mp4" }))
