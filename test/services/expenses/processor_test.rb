  ...
  class ProcessorTest < ActiveSupport::TestCase
    ...
    test "text input produces a normalized candidate without persisting anything" do
      assert_no_difference -> { Expense.count } do
        result = process(Expenses::Input.from_params("text", text: "Me gasté 50mil en almuerzos"))
        assert result.ok?
    ...
    test "text input with no detectable amount reports a clear error" do
      result = process(Expenses::Input.from_params("text", text: "hola que tal"))
    ...
    test "image input runs OCR + vision extraction and normalizes the result" do
      ...
      stub_vision_fallback(extractor_result) do
        result = process(Expenses::Input.from_params("image", image_data: "data:image/jpeg;base64,Zm9v"))
        assert result.ok?
...
result = process(Expenses::Input.from_params("text_image", text: "Este fue el recibo del almuerzo",
                                   image_data: "data:image/jpeg;base64,Zm9v"))
          assert result.ok?
    ...
    test "invalid inputs never reach extraction" do
      result = process(Expenses::Input.from_params("text", text: ""))
      assert_not result.ok?
      assert_nil result.candidate
      assert result.steps[:input].present?
      assert result.errors.any?

      result = process(Expenses::Input.from_params("image", image_data: "data:text/html;base64,PGI+"))
      assert_not result.ok?
    ...
    test "processing never creates a real expense" do
      stub_method(Ocr::LocalReader, :call, nil) do
        stub_method(Ai::ImageExpenseExtractor, :call,
          ...
          assert_no_difference -> { Expense.count } do
            process(Expenses::Input.from_params("image", image_data: "data:image/jpeg;base64,Zm9v"))
            process(Expenses::Input.from_params("text", text: "Compré gasolina por 120.000"))
          end
        end
      end
    end
