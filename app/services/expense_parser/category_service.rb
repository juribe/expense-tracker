  ...
  class CategoryService
    ...
    def self.resolve_category(description, context, categories)
      ...
      direct = categories.find do |category|
        name = TextService.normalize_text(category.name)
        normalized = TextService.normalize_text(description.to_s)
    ...
    def self.find_existing_category(group, haystack, categories)
      ...
      names.each do |name|
        hit = categories.find { |category| TextService.normalize_text(category.name) == TextService.normalize_text(name) }
        return hit if hit
      end
